...
bool petitRock(configuration *board) /// La réalisation du petit rock pour les blancs et les noires
{
    ...
    if((board->turn % 2) == 0)
    {
        if(board->roque[0] == 0)
        {
            if((board->board[0][5] == 0) && (board->board[0][6] == 0) && (board->board[0][4] == 'R') &&
               (board->board[0][7] == 'T') && (caseMenacer(*board, 1, 5) == 0) && (caseMenacer(*board, 1, 6) == 0) &&
                (caseMenacer(*board, 1, 7) == 0) && (caseMenacer(*board, 1, 8) == 0))
            {
...
int cout_possible (configuration t[], configuration config ,int type) /// Générer tous les couts possibles
{
    ...
    // pour chaque position positive (exemple)
    for(i=0;i<p;i++){
            // pour chaque position vide negative (exemple)
            for(k=0;k<p2;k++){
