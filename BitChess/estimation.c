...
// Compter le nombre des piéces (type=true -> les Blanches, type=false -> les Noires)
int nbrPieces(configuration board, bool type)
{
    int i, j, nbr = 0;
    if(type)
    {
    ...
    for(j = 0 ; j <= 7 ; j++)
    {
        for(i = 0 ; i <= 7 ; i++)
        {
            switch(board.board[i][j])
            {
                case 'P':
                    materiel += 100;
                    // récupere la val de la table prédifinie
